...
`+JSON.stringify(s)+`
`+JSON.stringify(n),u=await le(this.apiURL,{method:"POST",headers:{"Content-Type":"application/x-sentry-envelope"},body:i},r);this.log("Sentry response",u.status),u.status!==200&&(console.log(u.body),console.error("Failed to send event to Sentry",u))}catch(n){console.error("Failed to send event",n)}}async flush(e=5e3){try{this.log("Flushing sentry events",this.buffer.length);let r=this.buffer.splice(0,this.buffer.length).map(n=>this.sendEvent(n,e));await Promise.all(r),this.log("Flushed all events")}catch(r){console.error("Failed to flush events",r)}}static flush(e=5e3){return c.instance.flush(e)}static reset(){c._instance=void 0}},N(c,"_instance"),c);var de="https://d233059272824702afc8c43834c4912d@sentry.hcaptcha.com/6",pe="2.0.0",he="production";function D(t=!0){if(!t)return P();E.init({dsn:de,release:pe,environment:he});let e=E.scope;return I(e),P(e)}function P(t=null){return{addBreadcrumb:e=>{t&&t.addBreadcrumb(e)},captureRequest:e=>{t&&t.setRequest(e)},captureException:e=>{t&&E.captureException(e,t)}}}function M({scriptLocation:t,query:e,loadAsync:r=!0,crossOrigin:n,apihost:o="https://js.hcaptcha.com",cleanup:s=!0,secureApi:i=!1,scriptSource:u=""}={},h){let p=b(t),y=x(p);return new Promise((k,R)=>{let l=y.document.createElement("script");l.id=A,u?l.src=`${u}?onload=${f}`:i?l.src=`${o}/1/secure-api.js?onload=${f}`:l.src=`${o}/1/api.js?onload=${f}`,l.crossOrigin=n,l.async=r;let S=(m,B)=>{try{!i&&s&&p.removeChild(l),B(m)}catch(q){R(q)}};l.onload=m=>S(m,k),l.onerror=m=>{h&&h(l.src),S(m,R)},l.src+=e!==""?`&${e}`:"",p.appendChild(l)})}var w=[];function fe(t={cleanup:!0},e){try{e.addBreadcrumb({category:d,message:"hCaptcha loader params",data:t});let r=b(t.scriptLocation),n=x(r),o=w.find(({scope:i})=>i===n.window);if(o)return e.addBreadcrumb({category:d,message:"hCaptcha already loaded"}),o.promise;let s=new Promise(async(i,u)=>{try{n.window[f]=()=>{e.addBreadcrumb({category:d,message:"hCaptcha script called onload function"}),i(n.window.hcaptcha)};let h=C({custom:t.custom,render:t.render,sentry:t.sentry,assethost:t.assethost,imghost:t.imghost,reportapi:t.reportapi,endpoint:t.endpoint,host:t.host,recaptchacompat:t.recaptchacompat,hl:t.hl});await M({query:h,...t},p=>{e.captureRequest({url:p,method:"GET"})}),e.addBreadcrumb({category:d,message:"hCaptcha loaded",data:o})}catch{e.addBreadcrumb({category:d,message:"hCaptcha failed to load"});let p=w.findIndex(y=>y.scope===n.window);p!==-1&&w.splice(p,1),u(new Error(O))}});return w.push({promise:s,scope:n.window}),s}catch(r){return e.captureException(r),Promise.reject(new Error(O))}}async function U(t,e,r=0){let n=r<2?"Retry loading hCaptcha Api":"Exceeded maximum retries";try{return await fe(t,e)}catch(o){return e.addBreadcrumb({category:d,message:n}),r>=2?(e.captureException(o),Promise.reject(o)):(r+=1,U(t,e,r))}}async function F(t={}){let e=D(t.sentry);return await U(t,e)}
